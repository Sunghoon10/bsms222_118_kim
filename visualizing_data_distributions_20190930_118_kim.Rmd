...
```{r}
data(murders)
a <- murders %>% group_by(region) %>% count() %>% arrange(n)
barplot(a%>%pull()%>%prop.table(), ylab="Proportion", names.arg=a$region)
```
...
Pr(a<x<b)=∫ba 1/√2πs * e^-1/2{(x-m)/s}^2 dx
...
First look back at the formula for the normal distribution and note that what is being exponentiated is $-z^2/2$ with $z$ quivalent to
$x$ in standard units. Because the maximum of $e^-z^2/2$ is when $z = 0$, his explains why the maximum of the distribution occurs at the average. It also explains the symmetry since $-z^2/2$ s symmetric around 0. Second, note that if we convert the normally distributed data to standard units, we can quickly know if, for example, a person is about average $(z = 0)$, one of the largest $(z ≈ 2 )$, one of the smallest $(z ≈ -2)$ or an extremely rare occurrence ($z > 3$ or $z < -3$). Remember that it does not matter what the original units are, these rules apply to any data that is approximately normal.
...
```{r}
female_percentiles <- quantile(female, seq(1,2,3))
male_percentiles <- quantile(male, seq(1,2,3))
