...
그냥 summarize function을 쓰고 괄호 안에 표시하고 싶은 것들을 분류하여 적으면 data에 있는 값을 불러 정리해준다는 말인거 같다.
```{r}
heights %>% 
  filter(sex == "Female") %>%
...
murders %>% 
  ...
  head()
```
쉽게 말해서 보통 arrange를 쓰면 작은 값에서 부터 큰 값으로 배열이 되는데 desc를 쓰면 큰 값부터 배열되게 할 수 있다.
...
murders %>% 
  ...
  head()
```
 arrange 앞에 있는 걸로 먼저 배열한후 뒤의 기준을 가지고 추가적 배열을 한다.
...
data(NHANES)
```
나는 library로 설치가 안되기 때문에 Tools에서 실행하자
