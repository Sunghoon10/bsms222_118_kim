...
How can we compute $S_n$ fot various values of $n$, say $n=1,...,25$? Do we write 25 lines of code calling ```compute_s_n```? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of $n$. For-loops let us define the range that our variable takes (in our examole $n=1,...,10$), then change the value and evaluate expression as you loop.
...
s_n
```
처음 R Chunk에 ```s_n <- map_dbl(n, compute_s_n)``` 와 ```s_n``` 만을 넣었을 때 
```Error in map_dbl(n, compute_s_n) : could not find function "map_dbl"``` 라는 error가 떠서 goolging을 통해 ```library(purrr)```을 기입해주어야만 ```map_dbl``` function을 사용할 수 있음을 깨달았다.
